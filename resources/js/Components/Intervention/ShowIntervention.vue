              ...
              <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400 ">
                <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                  <tr>
                    ...
                    <th scope="col" class="py-3 px-6">Disponibilité</th>
                ...
                <tbody> 
                  <tr v-for="(article,index) in articles" :key="index" class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                    <td scope="row" class="py-4 px-6 font-medium text-gray-900 whitespace-nowrap dark:text-white">{{article.nom}}
                      <div v-if="article.commande && article.commande[0].status!='Annulée'" class="whitespace-nowrap">
                      <span class="inline-flex text-xs leading-5 font-semibold rounded-full p-1 bg-gray-100 text-gray-600">Commandé</span></div>
                    <div v-if="!article.commande || article.commande[0].status==='Annulée'" class="whitespace-nowrap">
                    <span class="inline-flex text-xs leading-5 font-semibold rounded-full p-1 bg-green-100 text-green-500">Stock</span></div>
                    </td>
                    <td scope="row" class="py-4 px-6 font-medium text-gray-900 whitespace-nowrap dark:text-white">{{article.Qt}}</td>
                    <td v-if="article.commande && article.commande[0].status!='Annulée'" class="py-7 flex flex-row items-center whitespace-nowrap">
                      <span v-if="article.commande[0].status==='En cours de préparation'" class="inline-flex text-xs leading-5 font-semibold rounded-full p-1 bg-orange-100 text-orange-500">En cours de préparation</span>
                      <span v-if="article.commande[0].status==='En cours de livraison'" class="inline-flex text-xs leading-5 font-semibold rounded-full p-1 bg-gray-100 text-gray-500">En cours de livraison</span>
                      <span v-if="article.commande[0].status==='Livrée'" class="inline-flex text-xs leading-5 font-semibold rounded-full p-1 bg-green-100 text-green-500">Livrée</span>
                      <Link :href="route('commandes.show',article.commande_id)" class="text-indigo-600 hover:text-indigo-900 pr-4"><svg xmlns="http://www.w3.org/2000/svg" width="17" height="17" fill="currentColor" class="bi ml-auto bi-info-circle" viewBox="0 0 16 16"> <path d="M8 15A7 7 0 1 1 8 1a7 7 0 0 1 0 14zm0 1A8 8 0 1 0 8 0a8 8 0 0 0 0 16z"/> <path d="m8.93 6.588-2.29.287-.082.38.45.083c.294.07.352.176.288.469l-.738 3.468c-.194.897.105 1.319.808 1.319.545 0 1.178-.252 1.465-.598l.088-.416c-.2.176-.492.246-.686.246-.275 0-.375-.193-.304-.533L8.93 6.588zM9 4.5a1 1 0 1 1-2 0 1 1 0 0 1 2 0z"/> </svg> </Link>
                    </td>
                    <td v-if="!article.commande || article.commande[0].status==='Annulée'" class="py-7 flex flex-row items-center whitespace-nowrap">
                    <span class="inline-flex text-xs leading-5 font-semibold rounded-full p-1 bg-green-100 text-green-500">Disponible</span>
                    </td>
                    <td scope="row" class="pt-5 pr-8 font-medium text-gray-900 whitespace-nowrap dark:text-white">
